...
class _AdminProjectManagementViewState
  ...
  void _addProject() async {
    ...
    final project = Project(
      ...
      totalProfit: totalBudget,
      minimumInvestment: roi,
        ...
        child: Column(
          ...
          children: [
            ...
            if (p.mediaPath != null)
              Center(
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(12),
                  child: kIsWeb
                      ? Image.network(p.mediaPath!, height: 160, width: double.infinity, fit: BoxFit.cover)
                      : Image.file(File(p.mediaPath!), height: 160, width: double.infinity, fit: BoxFit.cover),
                ),
              )
            else
              Container(
            ...
            Row(
              ...
              children: [
                Expanded(
                  child: Text(p.id.toString(), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF111712))),
                ),
                const SizedBox(width: 8),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                  decoration: BoxDecoration(
                    color: _statusColor(p.status.toString()).withOpacity(0.12),
                    borderRadius: BorderRadius.circular(20),
                  ),
                  child: Text(p.status.toString(), style: TextStyle(color: _statusColor(p.status.toString()), fontWeight: FontWeight.w600)),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text(p.description.toString(), style: const TextStyle(color: Colors.black87)),
            const SizedBox(height: 12),
            ...
            GridView.count(
              ...
              mainAxisSpacing: 6,
              children: [
                _smallInfo("Budget & ROI", "${(p.totalProfit ?? 0).toStringAsFixed(2)} MT / ${(p.profitabilityPercent ?? 0).toStringAsFixed(1)}%"),
                _smallInfo("Risco", p.riskLevel.toString(), valueColor: (p.riskLevel == 'Baixo') ? Colors.green : (p.riskLevel == 'Médio') ? Colors.orange : Colors.red),
                _smallInfo("Duração", "${_formatDate(DateTime(1))} • ${_formatDate(DateTime(1))}", valueColor: Colors.black87),
                _smallInfo("Total", "${(p.totalProfit ?? 0).toStringAsFixed(2)} MT"),
              ],
            ),
